...
public enum OperatingSystem {
        ...
        try {
            ...
            if (OperatingSystem.getCurrentPlatform() == OSX) {
                try {
                	...
                	bootstrap.logger.error("Failed to open link " + link.toString(), (Throwable)e1);
                }
            }
            bootstrap.logger.error("Failed to open link " + link.toString(), e);
        }
    }
    ...
    public static void openFolder(File path) {
        ...
        if (os == OSX) {
            try {
            	...
            	bootstrap.logger.error("Couldn't open " + path + " through /usr/bin/open", (Throwable)e);
            }
        } else if (os == WINDOWS) {
            ...
            try {
            	...
            	bootstrap.logger.error("Couldn't open " + path + " through cmd.exe", (Throwable)e);
            }
        }
        try {
            Class desktopClass = Class.forName("java.awt.Desktop");
        	...
        	bootstrap.logger.error("Couldn't open " + path + " through Desktop.browse()", e);
        }
    }
    ...
    public static boolean openHttpUrl(String url) {
		try {
			...
			bootstrap.logger.error(ex.getMessage());
	    }
		return false;
	}
}
